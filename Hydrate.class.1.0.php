...
class Hydrate
{
    ...
    function hydrateRelations($relations, $row, $hydratedRow, $localTable, $localTablePrefix)
    {
        $schema = $this->getSchema();
        ...
        foreach ($relations as $rel)
        {
            ...
            if ($relation["type"] != "one" OR empty($hydratedRow[$rel["name"]]))
            {
                ...
                foreach ($foreignTable["primary"] as $PKField)
                    if ($row["{$relationPrefix}_{$PKField}"] === NULL)
                    {
                        $PKFieldsNull = TRUE;
                        break;
                    }
                ...
                {
                    // For many to many relations we must make sure, that we are assigning foreign rows
                    // to a correct local row.
                    // We do that by checking local row id in the $hydratedRow array.
                    if (Hydrate_schema::isManyToMany($relation))
                    {
                        if ($row["{$rel["refTablePrefix"]}_{$relation["local"]}"] == $hydratedRow[$localTable["primary"][0]])
                            $doHydrate = TRUE;
                    }
                    else
                        $doHydrate = TRUE;
                ...
                if ($doHydrate)
                {
                    ...
                    $hydrateRes = $this->hydrateRow($row, $foreignTable, $relationPrefix, $rel, $hydratedRow[$rel["name"]]);
            ...
            if ($relation["type"] == "one")
            {
                ...
                if (isset($hydratedRow[$rel["name"]]))
                    $hydratedRow[$rel["name"]] = $this->hydrateRelations($rel["children"], $row, $hydratedRow[$rel["name"]], $foreignTable, $relationPrefix);
            }
            else
                foreach ($hydratedRow[$rel["name"]] as $k => $v)
                    $hydratedRow[$rel["name"]][$k] = $this->hydrateRelations($rel["children"], $row, $hydratedRow[$rel["name"]][$k], $foreignTable, $relationPrefix);
        }
